...
const accumulator = function (runningSum, number) {
  runningSum.push(runningSum.at(-1) + number);
  return runningSum;
}

const runningTotal = function ([first, ...rest]) {
  return rest.reduce(accumulator, [first])
}

const cumulativeSumsOf = function (arrays) { return arrays.map(runningTotal); }
...
const capitalizedFirstLettersOf = function (strings) {
  return strings.map(capitalizeWords);
}

// find word lengths in ["apple pie", "banana split"] => [[5, 3], [6, 5]]
const wordsLength = function (words) {
  return lengthsOf(words.split(' '));
}
...
const extractKey = extractInformaton('key');
const extractValue = extractInformaton('value');
const keyValuePair = compound(extractKey, extractValue);
const keyValuePairs = function (objects) { return objects.map(keyValuePair); }
...
const fullName = function (object) {
  return extractName(object).split(' ');
}
const splitFullNames = function (objects) { return objects.map(fullName); }
...
const testAll8 = function () {
  const result = [
    ...
    test('extractScores', extractScores, [[90,85], [80, 75]],
      [{ name: "Alice", scores: { math: 90, english: 85 } },
      { name: "Bob", scores: { math: 80, english: 75 } }]),
    test('keyValuePairs', keyValuePairs, [["a", 1], ["b", 2]],
      [{ key: "a", value: 1 }, { key: "b", value: 2 }]),
    test('splitFullNames', splitFullNames, [["Alice", "Smith"],
      ["Bob", "Brown"]], [{ name: "Alice Smith" }, { name: "Bob Brown" }]),
  ];
